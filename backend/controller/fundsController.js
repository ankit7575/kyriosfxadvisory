...
exports.addFortnightlyProfit = async (req, res) => {
  try {
    ...
    // Adding the profit entry to user's data
    user.fortnightlyProfit.push({
      history: [profitEntry],
    });

    // Function to add referral incentive to the user’s referral history
    const addReferralIncentive = async (referralUser, incentiveData) => {
      ...
      const incentiveEntry = {
        ...
        fortnightlyProfit: [{
          history: [{
            date: new Date(),
            fortnightlyProfit: profitAmount,
            stage3Incentive: incentiveData.stage3Incentive || 0,
            stage2Incentive: incentiveData.stage2Incentive || 0,
            directIncentive: incentiveData.directIncentive || 0,
          }]
        }],
        createdAt: new Date(),
      };

      // Process referral incentives for direct, stage2, and stage3
      const referralLevels = [
        { level: 'directReferral', incentive: incentiveData.directIncentive, stage: 1 },
        { level: 'stage2Referral', incentive: incentiveData.stage2Incentive, stage: 2 },
        { level: 'stage3Referral', incentive: incentiveData.stage3Incentive, stage: 3 }
      ];

      for (let { level, incentive } of referralLevels) {
        if (incentive) {
          // Find the existing referral entry for the correct user
          const referralHistory = referralUser.referral[level].find(ref => ref.user.toString() === user._id.toString());

          if (referralHistory) {
            // If referral history exists, add the new incentive entry
            referralHistory.history.push(incentiveEntry);
          } else {
            // If referral history doesn't exist, create a new entry
            referralUser.referral[level].push({
              user: user._id, 
              name: user.name, 
              history: [incentiveEntry],  
            });
          }
        }
      }
    ...
    const processReferralIncentives = async (currentUser, stage) => {
      ...
      // Fetch the referring user by referralId
      ...
      await addReferralIncentive(referrerUser, incentiveData);
    ...
    // Start processing referral incentives from stage 1
...
exports.viewReferralData = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const getUserDetails = async (userId) => {
      const user = await User.findById(userId).select("name phone email status role referral");
      if (user) {
        // Calculate total team size (number of referrals in all stages)
        const directTeamSize = user.referral.directReferral.length;
        const stage2TeamSize = user.referral.stage2Referral.length;
        const stage3TeamSize = user.referral.stage3Referral.length;

        const totalTeamSize = directTeamSize + stage2TeamSize + stage3TeamSize;
        ...
        return {
          ...
          status: user.status,
          role: user.role,
          teamSize: totalTeamSize  // Include total team size here
        };
      }
      return null;
    };
