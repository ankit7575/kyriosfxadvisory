...
const { sendUserDetailsEmail } = require('../utils/sendUserDetailsEmail'); // Import the new email utility
...
exports.registerUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (tempUserStore[email]) {
    // Instead of throwing an error, update the existing user's details if necessary
    // For example, you might want to update the OTP verification status or other information
    const existingUser = tempUserStore[email];
    existingUser.name = name;
    existingUser.phone = phone;
    existingUser.password = password;
    existingUser.referralbyId = referralId;
    existingUser.otpVerified = false;
    existingUser.createdAt = Date.now();
  } else {
    // If the user doesn't exist, create a new temporary user object
...
// Send the welcome email with OTP and registration PDF
await sendEmail({
  email: email,
  subject: "Welcome to Kyrios Fx Advisory - Referral Program Registration - Terms & Conditions",
  message: `Dear ${name},\n\nThank you for joining the Kyrios Fx Advisory Referral Program. Please use the OTP to complete your registration and activate your account. ${EMAIL_VERIFY_MESSAGE}`,
  attachments: [{ filename: `${name}_registration_with_otp.pdf`, path: pdfPath }],
});
...
res.status(201).json({
  ...
  otpSent: true,
  redirectTo: "/verification", // Redirect path to verification page
});
...
exports.verifyOtpAndCompleteRegistration = catchAsyncErrors(async (req, res, next) => {
  ...
  await sendUserDetailsEmail(newUser, tempUser);
...
exports.loginUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user.emailVerified) {
    return next(new ErrorHandler("Email not verified. Please verify to log in.", 401));
  }
...
exports.AdmindeleteUser = catchAsyncErrors(async (req, res, next) => {
  ...
  await User.findByIdAndDelete(req.params.id); // Correct method to delete the user
