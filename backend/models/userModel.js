...
userSchema.methods.requestTraderAccount = function () {
  if (this.traderAccountStatus === 'approved' || this.traderAccountStatus === 'rejected') {
    throw new Error('Trader account request has already been processed.');
  }

  this.requestedTraderStatus = true;
  this.traderAccountStatus = 'requested';
  return this.save();
};

userSchema.methods.verifyTraderAccount = function (status) {
  if (this.traderAccountStatus !== 'requested') {
    throw new Error('Trader account request must be in "requested" status to be verified.');
  }

  if (status === 'approved') {
    this.traderAccountStatus = 'approved';
    this.role = 'trader'; // Change the user's role to trader upon approval
  } else if (status === 'rejected') {
    this.traderAccountStatus = 'rejected';
  } else {
    throw new Error('Invalid status provided for trader account verification.');
  }
