...
const sendEmail = async (options) => {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`Email sent to: ${options.email}`);  // Allow console in non-production environments
    }
  } catch (error) {
    if (process.env.NODE_ENV !== 'production') {
      console.error('Error sending email:', error);  // Allow console in non-production environments
    }
...
const sendOtp = async (email) => {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`OTP sent to: ${email}`);  // Allow console in non-production environments
    }
  } catch (error) {
    if (process.env.NODE_ENV !== 'production') {
      console.error('Error sending OTP:', error);  // Allow console in non-production environments
    }
...
const verifyOtp = (email, otp) => {
  ...
  if (entry && entry.otp === otp && entry.expires > Date.now()) {  // Use strict equality (===)
